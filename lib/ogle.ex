defmodule Ogle do
  ...
  To emit Statsd metrics, `Ogle` supports both UDP and Unix Domain Sockets.

  ## Why another `Telemetry.Metrics` library?

  Both `TelemetryMetricsStatsd` and `TelemetryMetrics.Prometheus` are great
  choices for emitting telemetry. However, `Ogle` makes several different
  ...
  shapes of distributions. `Ogle` represents distributions using histograms,
  ...
  `Ogle` uses `:atomics` stored in `:ets` for performance. New `:atomics` arrays
  ...
  tags. `Ogle` reporter processes are not involved in the handling of any
  ...
  histograms. There is no step in `Ogle`'s processing where samples are
  ...
  each reporting period, metrics collected by a `Ogle` reporter will be
  collected into a minimum number of packets. Users can set the maximum packet
  size in `Ogle.Options`.
  ...
  use GenServer
  require Logger
  alias Ogle.{EventHandler, Options, Statsd}
  ...
  def child_spec(options) do
    %{id: ogle_name!(options), start: {__MODULE__, :start_link, [options]}}
  end
  ...
  def insert_metric(name, metric, value, tags) when is_number(value) do
    case Ogle.Persistent.fetch(name) do
      %Ogle.Persistent{
  ...
  @doc """
  Returns measurements about the size of a running Ogle's storage, in number of
  ...
  def storage_size(name) do
    case Ogle.Persistent.storage(name) do
      {storage_mod, storage} ->
        measurements = storage_mod.storage_size(storage)

        Ogle.Telemetry.storage_size(measurements, name, storage_mod)
  ...
  def get_all_metrics(name) do
    case Ogle.Persistent.fetch(name) do
      %Ogle.Persistent{storage: {storage_mod, storage}} = p ->
  ...
  def init(options) do
    ...
    :ok =
      Ogle.Persistent.new(options)
      |> Ogle.Persistent.store()

    :ok = Ogle.Codegen.create(options)
  ...
  def handle_info(_msg, state) do
    ...
    # socket blocks, and Ogle should not terminate the server and lose state when that
    # happens.
    #
  ...
  def terminate(_reason, %{name: name, handler_ids: handler_ids}) do
    Ogle.Codegen.purge(name)
    Ogle.Persistent.erase(name)
